...
async function markSent(conn, ids) {
  ...
  const sql = `
    UPDATE ${process.env.DB_TABLE2}
    SET enviado = 1
    WHERE idalertas IN (${placeholders})
  `;
  console.log(sql);
...
function tableHtml(rows) {
  ...
  const columnas = [
    { key: 'idalertas', label: 'ID Alerta' },
    { key: 'sku', label: 'SKU' },
    { key: 'nombre', label: 'Descripción' },
    { key: 'bod', label: 'Cod Bodega' },
    { key: 'descripcion', label: 'Bodega' },
    { key: 'estado', label: 'Estado' },
    { key: 'actualizacion', label: 'Fecha' },
  ];
...
async function sendHtmlEmails(rows) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    const part = chunks[i];
    const subjectBase = 'Reporte alertas Habib Droguerias';
...
async function runOnce2() {
  ...
  try {
    ...
    // marcar como enviado
    const ids = rows.map(r => r.idalertas).filter(v => v !== undefined && v !== null);
